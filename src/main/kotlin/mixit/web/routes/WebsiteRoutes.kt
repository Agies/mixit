...
class WebsiteRoutes(private val adminHandler: AdminHandler,
    ...
    fun websiteRouter() = router {
        ...
        accept(TEXT_HTML).nest {
            GET("/") { sponsorHandler.viewWithSponsors("home", null, 2018, it) }
            GET("/about", globalHandler::findAboutView)
            GET("/news", newsHandler::newsView)
            GET("/ticketing", ticketingHandler::ticketing)
            GET("/sponsors") { sponsorHandler.viewWithSponsors("sponsors", "sponsors.title", 2018, it) }
            GET("/mixteen", { sponsorHandler.viewWithSponsors("mixteen", "mixteen.title", 2018, it) })
            ...
            eventRepository.findAll().toIterable().map { it.year }.forEach { year ->
                GET("/sponsors/$year") { sponsorHandler.viewWithSponsors("sponsors", "sponsors.title", year, it) }
            }
